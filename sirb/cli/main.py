...
let _runDoneFired=false;const sseEl=document.getElementById("sse-status"),evtSource=new EventSource("/events");
...
evtSource.onmessage=(e)=>{try{const d=JSON.parse(e.data);if(d.type=="stats"&&d.data){if(!currentRunId||d.data.run_id!==currentRunId)return;liveStats(d.data);if(d.data.Status=="done"&&!_runDoneFired){_runDoneFired=true;setTimeout(()=>selectRun(currentRunId),500)}}}catch(_){}};
...
async function handleLaunchResponse(p){document.getElementById("run-btn").disabled=true;document.getElementById("run-btn").textContent="Running...";document.getElementById("stop-btn").style.display="inline-block";try{const r=await p;const d=await r.json();if(d.run_id){currentRunId=d.run_id;_userScrolledSIRB=false;document.getElementById("sirb-hero").style.display="none";document.getElementById("selected-run").textContent="Run: "+d.run_id;reportCache={};document.getElementById("report-tabs").style.display="none";document.getElementById("report-tabs").innerHTML="";document.getElementById("final-summary").style.display="none";document.getElementById("live-stats").innerHTML="";document.getElementById("agent-cards").innerHTML="";_runDoneFired=false;document.getElementById("assessment-view").innerHTML='<span style="color:var(--accent)">⏳ Run started... agents initializing.</span>';setTimeout(loadRuns,1000)}else if(d.error){alert("Error: "+d.error)}}catch(e){alert("Failed: "+e)}document.getElementById("run-btn").disabled=false;document.getElementById("run-btn").textContent="▶ Run"}
